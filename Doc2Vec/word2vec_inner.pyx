...
ctypedef void (*fast_sentence_sg_hs_ptr) (
    ...
    REAL_t *context, REAL_t *syn1, const int size,
...
ctypedef unsigned long long (*fast_sentence_sg_neg_ptr) (
    ...
    REAL_t *context, REAL_t *semantic, const int size, const np.uint32_t word_index,
    const np.uint32_t word2_index, const REAL_t alpha, REAL_t *work,
    unsigned long long next_random) nogil

ctypedef void (*fast_sentence_cbow_hs_ptr) (
    const np.uint32_t *word_point, const np.uint8_t *word_code, int codelens[MAX_SENTENCE_LEN],
    REAL_t *neu1,  REAL_t *context, REAL_t *syn1, const int size,
...
ctypedef unsigned long long (*fast_sentence_cbow_neg_ptr) (
    ...
    REAL_t *neu1,  REAL_t *context, REAL_t *semantic, const int size,
    np.uint32_t indexes[MAX_SENTENCE_LEN], const REAL_t alpha, REAL_t *work,
...
cdef void fast_sentence0_sg_hs(
    ...
    for b in range(codelen):
        ...
        f = <REAL_t>dsdot(&size, &context[row1], &ONE, &syn1[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
        saxpy(&size, &g, &syn1[row2], &ONE, work, &ONE)
        saxpy(&size, &g, &context[row1], &ONE, &syn1[row2], &ONE)
    saxpy(&size, &ONEF, work, &ONE, &context[row1], &ONE)
...
cdef void fast_sentence1_sg_hs(
    ...
    for b in range(codelen):
        row2 = word_point[b] * size
        f = <REAL_t>sdot(&size, &context[row1], &ONE, &syn1[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (1 - word_code[b] - f) * alpha
...
cdef void fast_sentence2_sg_hs(
    ...
    for b in range(codelen):
        ...
        for a in range(size):
            syn1[row2 + a] += g * context[row1 + a]
    for a in range(size):
        context[row1 + a] += work[a]
...
cdef unsigned long long fast_sentence0_sg_neg(
    ...
    for d in range(negative+1):
        ...
        f = <REAL_t>dsdot(&size, &context[row1], &ONE, &semantic[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
        saxpy(&size, &g, &semantic[row2], &ONE, work, &ONE)
        saxpy(&size, &g, &context[row1], &ONE, &semantic[row2], &ONE)
...
cdef unsigned long long fast_sentence1_sg_neg(
    ...
    for d in range(negative+1):
        ...
        row2 = target_index * size
        f = <REAL_t>sdot(&size, &context[row1], &ONE, &semantic[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
...
cdef unsigned long long fast_sentence2_sg_neg(
    ...
    for d in range(negative+1):
        ...
        f = <REAL_t>0.0
        for a in range(size):
            f += context[row1 + a] * semantic[row2 + a]
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
        for a in range(size):
            work[a] += g * semantic[row2 + a]
        for a in range(size):
            semantic[row2 + a] += g * context[row1 + a]
...
cdef void fast_sentence0_cbow_hs(
    ...
    for m in range(j, k):
        if m == i or codelens[m] == 0:
            ...
            count += ONEF
            saxpy(&size, &ONEF, &context[indexes[m] * size], &ONE, neu1, &ONE)
    ...
    for m in range(j, k):
        if m == i or codelens[m] == 0:
            ...
            saxpy(&size, &ONEF, work, &ONE, &context[indexes[m] * size], &ONE)
...
cdef void fast_sentence2_cbow_hs(
    ...
    for m in range(j, k):
        if m == i or codelens[m] == 0:
            ...
            count += ONEF
            for a in range(size):
                neu1[a] += context[indexes[m] * size + a]
    if cbow_mean and count > (<REAL_t>0.5):
        for a in range(size):
            neu1[a] /= count
    ...
    for m in range(j, k):
        if m == i or codelens[m] == 0:
            ...
            for a in range(size):
                context[indexes[m] * size + a] += work[a]
...
cdef unsigned long long fast_sentence0_cbow_neg(
    ...
    for d in range(negative+1):
        ...
        row2 = target_index * size
        f = <REAL_t>dsdot(&size, neu1, &ONE, &semantic[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
        saxpy(&size, &g, &semantic[row2], &ONE, work, &ONE)
        saxpy(&size, &g, neu1, &ONE, &semantic[row2], &ONE)
...
cdef unsigned long long fast_sentence1_cbow_neg(
    ...
    for d in range(negative+1):
        ...
        row2 = target_index * size
        f = <REAL_t>sdot(&size, neu1, &ONE, &semantic[row2], &ONE)
        if f <= -MAX_EXP or f >= MAX_EXP:
            continue
        f = EXP_TABLE[<int>((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]
        g = (label - f) * alpha
...
def train_sentence_sg(model, sentence, alpha, _work):
    ...
    cdef REAL_t *context = <REAL_t *>(np.PyArray_DATA(model.context))
    cdef REAL_t *work
    ...
    if negative:
        semantic = <REAL_t *>(np.PyArray_DATA(model.semantic))
        ...
        for i in range(sentence_len):
            ...
            for j in range(j, k):
                if j == i or codelens[j] == 0:
                    continue
                if hs:
                    fast_sentence_sg_hs(points[i], codes[i], codelens[i], context, syn1, size, indexes[j], _alpha, work)
                if negative:
                    next_random = fast_sentence_sg_neg(negative, table, table_len, context, semantic, size, indexes[i], indexes[j], _alpha, work, next_random)
    ...
    with nogil:
        for i in range(sentence_len):
            ...
            if k > sentence_len:
                k = sentence_len
            if hs:
                fast_sentence_cbow_hs(points[i], codes[i], codelens, neu1, context, syn1, size, indexes, _alpha, work, i, j, k, cbow_mean)
            if negative:
                next_random = fast_sentence_cbow_neg(negative, table, table_len, codelens, neu1, context, semantic, size, indexes, _alpha, work, i, j, k, cbow_mean, next_random)
