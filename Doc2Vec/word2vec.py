            ...
            for pos, word in enumerate(sentence):
                ...
                for pos2, word2 in enumerate(sentence[start : pos + model.window + 1 - reduced_window], start):
                    ...
                    if word2 and not (pos2 == pos):
                        ...
                        model.context[word2.index] += neu1e  # learn input -> hidden
...
class Word2Vec(utils.SaveLoad):
    ...
    def build_vocab(self, sentences, reset=True):
        """
    ...
    def reset_weights(self):
        ...
        self.context = empty((len(self.vocab), self.layer1_size), dtype=REAL)
        # randomize weights vector by vector, rather than materializing a huge random matrix in RAM at once
        for i in xrange(len(self.vocab)):
            self.context[i] = (random.rand(self.layer1_size) - 0.5) / self.layer1_size
        if self.hs:
            self.syn1 = zeros((len(self.vocab), self.layer1_size), dtype=REAL)
        if self.negative:
            self.semantic = zeros((len(self.vocab), self.layer1_size), dtype=REAL)
        self.contextnorm = None
    ...
    def save_word2vec_format(self, fname, fvocab=None, binary=False):
        ...
        assert (len(self.vocab), self.layer1_size) == self.context.shape
        with utils.smart_open(fname, 'wb') as fout:
            fout.write(utils.to_utf8("%s %s\n" % self.context.shape))
            # store in sorted order: most frequent words at the top
            for word, vocab in sorted(iteritems(self.vocab), key=lambda item: -item[1].count):
                row = self.context[vocab.index]
                if binary:
    ...
    def load_word2vec_format(cls, fname, fvocab=None, binary=False, norm_only=True):
        ...
        with utils.smart_open(fname) as fin:
            ...
            result.context = zeros((vocab_size, layer1_size), dtype=REAL)
            if binary:
                ...
                for line_no in xrange(vocab_size):
                    ...
                    result.context[line_no] = fromstring(fin.read(binary_len), dtype=REAL)
            else:
                for line_no, line in enumerate(fin):
                    ...
                    if counts is None:
                        ...
                        result.vocab[word] = Vocab(index=line_no, count=None)
                    result.index2word.append(word)
                    result.context[line_no] = weights
        logger.info("loaded %s matrix from %s" % (result.context.shape, fname))
    ...
    def most_similar(self, positive=[], negative=[], topn=10):
        ...
        for word, weight in positive + negative:
            if isinstance(word, ndarray):
                ...
                mean.append(weight * self.contextnorm[self.vocab[word].index])
        ...
        dists = dot(self.contextnorm, mean)
        if not topn:
            return dists
    ...
    def doesnt_match(self, words):
        ...
        vectors = vstack(self.contextnorm[self.vocab[word].index] for word in words).astype(REAL)
    ...
    def __getitem__(self, word):
        ...
        """
        return self.context[self.vocab[word].index]
    ...
    def init_sims(self, replace=False):
        ...
        """
        if getattr(self, 'contextnorm', None) is None or replace:
            logger.info("precomputing L2-norms of word weight vectors")
            if replace:
                for i in xrange(self.context.shape[0]):
                    self.context[i, :] /= sqrt((self.context[i, :] ** 2).sum(-1))
                self.contextnorm = self.context
                if hasattr(self, 'syn1'):
                    del self.syn1
            else:
                self.contextnorm = (self.context / sqrt((self.context ** 2).sum(-1))[..., newaxis]).astype(REAL)
    ...
    def save(self, *args, **kwargs):
        kwargs['ignore'] = kwargs.get('ignore', ['contextnorm'])  # don't bother storing the cached normalized vectors
        ...
        for pos, word in enumerate(sentence):
            ...
            for pos2, word2 in enumerate(sentence[start : pos + model.window + 1 - reduced_window], start):
                ...
                if word2:
                    ...
                    # model.context[word.index] += neu1e  # learn input -> hidden
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 3:
        ...
        out_file = sys.argv[3]
        model = Doc2Vec(LineSentence(input_file), model_file=model_file, iteration=100)
